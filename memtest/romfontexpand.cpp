...
void romfontExpand()
{
    ...
    {
        fontHeader = (u8 *) __memAlloc(Heap::HEAP_MAP, 0x90ee4);
        temp = (u8 *) __memAlloc(Heap::HEAP_MAP, 0x4d000);
    }
    else
    {
        fontHeader = (u8 *) __memAlloc(Heap::HEAP_MAP, 0x10120);
        temp = (u8 *) __memAlloc(Heap::HEAP_MAP, 0x3000);
    }
    ...
    RomfontEntry * newEntries = (RomfontEntry*) __memAlloc(Heap::HEAP_MAIN, sizeof(RomfontEntry) * romfont_wp->entryCount);
    ...
    __memFree(Heap::HEAP_MAP, fontHeader);
    __memFree(Heap::HEAP_MAP, temp);

    // Replace old entry array
    __memFree(Heap::HEAP_MAIN, romfont_wp->entries);
